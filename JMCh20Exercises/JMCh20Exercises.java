...
public class JMCh20Exercises<E>
{
    ...
    {
        int i;
        for (i = 0; head != null; head = head.getNext()){
            i++;
        }
    ...
    {
        ListNode<E> newHead = null;

        for (ListNode<E> n = head; n != null; n = n.getNext())
        {
          ListNode<E> tempNode = new ListNode<E>(n.getValue(), newHead);
          newHead = tempNode;
        }
    ...
    {
        ListNode<String> node = head, prev = null;
    ...
    {
        ListNode<String> headHolder = head;

        while (headHolder != null)
        {
          ListNode<String> minNode = headHolder;
          for (ListNode<String> node = headHolder.getNext(); node != null; node = node.getNext())
          {
            String str = (String)node.getValue();
            if (str.compareTo((String)minNode.getValue()) < 0)
              minNode = node;
          }
          
          Object min = minNode.getValue();
          minNode.setValue(headHolder.getValue());
          headHolder.setValue((String)min);

          headHolder = headHolder.getNext();
        }
    }
    ...
    { 
        ListNode2<Integer> max = new ListNode2<Integer>(Integer.MIN_VALUE,null, null);
        ListNode2<Integer> headHolder = head;
        
        while (head != null){
            if (head.getValue().compareTo( max.getValue() ) > 0){
                max = head;
            }
            head = head.getNext();
        }
        
        ListNode2<Integer> nextHead = max.getNext();
        ListNode2<Integer> beforeHead = max.getPrevious();
        nextHead.setPrevious( beforeHead );
        beforeHead.setNext( nextHead );
        max.setNext( null );
        max.setPrevious( null );
        
        if (headHolder.getValue() == max.getValue()){
            return nextHead;
        }
